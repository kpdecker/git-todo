            ...
            aheadBehind(info.branch, file, true, function(err, aheadBehind) {
              ...
              localStatus(file, function(err, status) {
                ...
                unmergedBranches(file, function(err, branches) {
                  ...
                  info.unmergedBranches = branches;
...
function unmergedBranches(path, callback) {
  childProcess.exec('git branch --no-merged', {cwd: path}, function(err, branches) {
    ...
    var ret = {},
        hasBranches;
    async.eachSeries(branches, function(branch, callback) {
        if (!branch) {
          return callback();
        }

        aheadBehind(branch, path, false, function(err, info) {
          if (!err) {
            ret[branch] = info;
            hasBranches = true;
          }
